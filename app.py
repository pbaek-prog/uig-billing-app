...
# ==============================
    ...
    st.markdown('<p class="sub-header">Auto-sync deadlines and court dates to Google Calendar</p>', unsafe_allow_html=True)
    st.divider()

    try:
        import calendar_sync as cs
    except Exception as e:
        st.error(f"Failed to load calendar_sync module: {e}")
        cs = None

    st.info(
        "⚠️ **OAuth re-authorization required on first use**: Calendar access scope has been added. "
        "Please re-authorize on the Google Workspace Setup page."
    )
    ...
    if cs and USE_GOOGLE_SHEETS:
        try:
            ...
            with col1:
                if st.button("📋 List Available Calendars"):
                    ...
                    if cals:
                        ...
                        st.warning("Unable to retrieve calendar list. Please check OAuth scopes.")

            with col2:
                calendar_id_input = st.text_input(
                    "Target Calendar ID",
                    value="primary",
                    help="Default 'primary' is the main calendar"
                )

            st.divider()
            st.subheader("Bulk Sync Deadlines")

            if st.button("🔄 Sync All Pending Deadlines to Calendar", type="primary"):
                ...
                st.success(
                    f"✅ Sync complete — Added: {result.get('synced', 0)}, "
            ...
            st.divider()
            st.subheader("Add Individual Event")

            with st.form("manual_calendar_event"):
                ev_title = st.text_input("Event Title*", placeholder="USCIS Interview")
                ev_date = st.date_input("Date*", value=date.today())
                ev_client = st.text_input("Client Name")
                ev_case = st.text_input("Case Number")
                ev_location = st.text_input("Location", placeholder="USCIS Chicago Field Office")
                ev_desc = st.text_area("Description", height=80)
                ev_submit = st.form_submit_button("Add to Calendar", type="primary")
            ...
            if ev_submit and ev_title:
                ...
                if res.get("success"):
                    ...
                    if res.get("html_link"):
                        st.markdown(f"[📅 View in Calendar]({res['html_link']})")
                else:
                    st.error(f"Failed: {res.get('error')}")

        except Exception as e:
            st.error(f"Calendar Sync initialization error: {e}")
            st.info("Re-authorization may be required on the Google Workspace Setup page.")
    else:
        st.warning("Google Sheets connection required.")
...
# ==============================
    ...
    st.markdown('<p class="sub-header">Electronic signature request and management</p>', unsafe_allow_html=True)
    st.divider()

    try:
        import esignature as esig
    except Exception as e:
        st.error(f"Failed to load esignature module: {e}")
        esig = None
    ...
    if esig and USE_GOOGLE_SHEETS:
        try:
            ...
            tab_new, tab_pending, tab_sign = st.tabs([
                "📤 Create Request", "⏳ Pending Requests", "✍️ Attorney Sign"
            ])

            with tab_new:
                st.subheader("Send Signature Request to Client")
                with st.form("new_signature_request"):
                    sr_client_id = st.text_input("Client ID*", placeholder="CLIENT-2026-001")
                    sr_client_name = st.text_input("Client Name*")
                    sr_doc_name = st.text_input("Document Name*", placeholder="Retainer Agreement.pdf")
                    sr_doc_id = st.text_input("Drive File ID*", help="Google Drive file ID of the PDF to be signed")
                    sr_notes = st.text_area("Notes", height=60)
                    sr_submit = st.form_submit_button("Create Signature Request", type="primary")
                ...
                if sr_submit and sr_client_id and sr_client_name and sr_doc_id:
                    try:
                        ...
                        st.success(f"✅ Signature request created: `{req_id}`")
                        st.info("Client can view and sign in the 'Sign Documents' tab after logging into the Client Portal.")
                    except Exception as e:
                        st.error(f"Error: {e}")

            with tab_pending:
                st.subheader("Pending Signature Requests")
                ...
                if pending:
                    ...
                    st.info("No pending signature requests.")

            with tab_sign:
                st.subheader("Attorney Direct Signing (Insert Signature into PDF)")
                st.caption("Upload a PDF, sign on the pad below, then download the signed PDF.")

                uploaded_pdf = st.file_uploader("Upload PDF", type=["pdf"], key="sign_pdf_upload")

                col_a, col_b = st.columns(2)
                with col_a:
                    sig_x = st.number_input("X Position (points)", value=100, min_value=0)
                    sig_y = st.number_input("Y Position (points)", value=100, min_value=0)
                with col_b:
                    sig_w = st.number_input("Width (points)", value=150, min_value=20)
                    sig_h = st.number_input("Height (points)", value=50, min_value=10)

                sig_page = st.number_input("Page Number (0=first page, -1=last page)", value=-1, min_value=-1)
                signer_name = st.text_input("Signer Name", value="US Immigration Group")

                st.write("**Sign in the area below using mouse or touch:**")
                sig_img = esig.signature_canvas_component(key="admin_sig_pad")

                if uploaded_pdf and sig_img and st.button("🖋️ Insert Signature & Download PDF", type="primary"):
                    try:
            ...
            st.error(f"e-Signature initialization error: {e}")
    else:
        st.warning("Google Sheets connection required.")
